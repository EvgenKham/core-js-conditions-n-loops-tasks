...
function convertToRomanNumerals(num) {
  ...
  const one = Math.floor(num / 10) * 10;
  const second = num - one;

  return roman[one] + roman[second];
}
...
function isPalindrome(str) {
  ...
  for (let start = 0; start < str.length; start += 1) {
    const end = str[str.length - 1 - start];
    if (str[start] === end) {
      isResult = true;
    } else {
      isResult = false;
...
function shuffleChar(str, iterations) {
  ...
  for (let count = 0; iter > 0; count += 1) {
    if (temp === str && count > 0) {
      iter %= count;
    }
...
function getNearestBigger(number) {
  ...
  if (!currentIndex) return number;

  for (let i = digits.length - 1; i >= 0; i -= 1) {
    if (digits[i] > digits[currentIndex]) {
      const temp = digits[i];
  ...
  for (let i = currentIndex + 1; i < digits.length - 1; i += 1) {
